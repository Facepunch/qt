...
{
    concat(exactly-one($inputDocument/xml/File[@id = string(exactly-one($block/@file))]/@name), " at line ", string(exactly-one($block/@line)))
};

declare function local:report() as element()+
{
            let $complexVariables as element(Variable)* := $inputDocument/xml/Variable[local:isComplexType(exactly-one(@type))]
            ...
            let $primitiveVariables as element(Variable)+ := one-or-more($inputDocument/xml/Variable[local:isPrimitive(exactly-one(@type))])
