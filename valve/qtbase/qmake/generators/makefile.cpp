...
// VALVE-BEGIN
    ...
    {
        //
        // Add our custom allocator hooks to all non-lib Qt projects.
        //
        // The first draft of this change (based on CL#810984) was much simpler.
        //
        // However the 'verifyspec' build that happens very early during configure
        // where QT_SOURCE_TREE doesn't seem to be defined. It appears to be the case that
        // sourceRoot() is predictable and just up a dir from the normal QT_SOURCE_TREE
        // (and with a trailing slash)
        // The builds outside of /qtbase/ also don't set up in a predictable 
        // So we just require /valve to be in the source root path and go from there
        //
        // The 'libGLESv2' and 'macextras' were other projects that made this code
        // more complex than it needed to be. Test them if you want to change it!
        //
        QString srcTree;
        if( !project->values("QT_SOURCE_TREE").isEmpty() )
        {
            srcTree = project->first("QT_SOURCE_TREE").toQString();
        }

        QString valveDirParent;
        QStringList sourceRootSplit = projectFile()->sourceRoot().replace( "\\", "//" ).split( QRegExp( "/valve" ), Qt::KeepEmptyParts );
        if ( sourceRootSplit.count() == 2 )
        {
            valveDirParent = sourceRootSplit[ 0 ];
        }
        else
        {
            // inject an obvious bad path to get their attention
            valveDirParent = "__COULD_NOT_FIND_VALVE_IN_CODE_PATH__";
        }
     
        QString srcPath = valveDirParent + "/valve/qtbase/src/valve_memoverride/valve_memoverride.cpp";
        ...
        QString objPath = Valve_ObjectDir( srcPath, objdir );
        if (project->isActiveConfig("win32"))
        {
            objPath += "valve_memoverride.obj";
        }
        else
        {
        ...
        projectFile()->values("SOURCES") += Option::fixPathToTargetOS( srcPath );
        projectFile()->values("OBJECTS") += Option::fixPathToTargetOS( objPath );
    }
// VALVE-END
}
...
// VALVE-BEGIN
//
// We factored this function out of createObjectList so it could be called above,
// because there didn't seem to otherwise be a reliable way to predict the correct
// path for an OBJ to inject into the project above. This was really only a problem
// for projects with the 'object_parallel_to_source' flag.
//
QString MakefileGenerator::Valve_ObjectDir( QString sfn, QString objdir )
{
    QString dir;
...
// VALVE-BEGIN
        QString dir = Valve_ObjectDir( sfn, objdir );
// VALVE-END
